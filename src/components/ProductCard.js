  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <p className="card-price">{`R$ ${price ? price.toFixed(2).replace(/\./gm, ',') : 'Sob consulta'}`}</p>

        {freeShipping && <p className="card-ship" data-testid="free-shipping"><FaBox />Frete grátis</p>}
        <Link className="card-link" data-testid="product-detail-link" to={ `/product/${id}` }>
          Detalhes do produto
        </Link>
